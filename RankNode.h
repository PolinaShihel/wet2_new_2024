...
public:
    ...
    RankNode(Cond key, T data, int rank, int extra) : key(key), right(nullptr), left(nullptr), height(0),
                                                      data(data), rank(rank), extra(extra), extraW(key),
                                                      bigExtraW(this), amount(0) {};

    RankNode(Cond key, T data, int rank, int extra, int extraW) : key(key), right(nullptr), left(nullptr), height(0),
                                                                  data(data), rank(rank), extra(extra), extraW(extraW),
                                                                  amount(0) {}; //NOT SURE NEEDED
...
void RankNode<T, Cond>::updateAmount() {
    ...
    this->extra = 0;
//    if(this->bigExtraW->key == this->key){
//        this->extraW = this->key + this->amount;
//    }
}
...
RankNode<T, Cond> *RankNode<T, Cond>::findClosestSmall(Cond &toFind, RankNode<T, Cond> *current) {
    try {
        ...
        this->updateAmount();
        if (this->key < toFind)
            temp = this->right->findClosestSmall(toFind, current);
        else
            temp = this->left->findClosestSmall(toFind, this);
...
RankNode<T, Cond> *RankNode<T, Cond>::findClosestBig(Cond &toFind, RankNode<T, Cond> *current) {
    try {
        ...
        this->updateAmount();
        if (this->key < toFind)
            temp = this->right->findClosestBig(toFind, this);
        else
            temp = this->left->findClosestBig(toFind, current);
        this->findMax(this->key, this->amount);
        return temp;
    } catch (...) {
        if(this != nullptr)
...
int RankNode<T, Cond>::Rank(Cond &toFind) {
    try {
        ...
        if (this->key < toFind) {
            if (this->left != nullptr)
                temp = this->left->rank + 1 + this->right->Rank(toFind);
            else
                temp = 1 + this->right->Rank(toFind);
        } else if (this->key > toFind)
            temp = this->left->Rank(toFind);
        else {
            if (this->left != nullptr)
                temp = this->left->rank + 1;
            else temp = 1;
        }
        this->findMax(this->key, this->amount);
        return temp;
...
T *RankNode<T, Cond>::select(int rank, int sum) {
    try {
        ...
        T* temp;
        if (thisRank < rank)
            temp =  this->right->select(rank, thisRank);
        else if (thisRank > rank)
            temp =  this->left->select(rank, sum);
        else
            temp =  this->getNodeDataPointer();
        this->findMax(this->key, this->amount);
        return temp;
    } catch (...) {
        if(this != nullptr)
...
int RankNode<T, Cond>::findSum(const Cond &toFind, int sum) {
    try {
        ...
        this->updateAmount();
        if (this->key < toFind)
            temp =  this->right->findSum(toFind, sum);
        else if (this->key > toFind)
            temp =  this->left->findSum(toFind, sum);
        else
            temp =  this->amount;
        this->findMax(this->key,this->amount);
        return temp;
...
void RankNode<T, Cond>::inOrderTraversal(int depth) {
    ...
    std::cout << "key: " << this->key << " rank: " << this->rank << " extra: " << this->extra << " depth: " << depth
              << std::endl;
...
//    }
    ...
    if (this->right != nullptr && this->right->extraW + this->right->extra > this->extraW) {
        this->extraW = this->right->extraW + this->right->extra;
